...
class SyntaxAnalyzer {
  ...
  val ADJECTIVES : List[String] = List("fat", "happy", "mean", "hungry")
  ...
  // This method implements the BNF rule for an adjective <adjective>  ::= fat | happy | mean | hungry
  def Adjective() = {
  ...
  def Adverb() = {
    if (ADVERBS contains Compiler.currentToken)
      Compiler.Scanner.getNextToken()
//    else {
//      println("SYNTAX ERROR - A adverb was expected when '" + Compiler.currentToken + "' was found.")
//      setError()
//    }
  }
